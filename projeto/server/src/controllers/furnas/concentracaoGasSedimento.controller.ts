...
// Mapeamento
const mapRowToConcentracaoGasSedimento = (row: any) => ({
    idConcentracaoGasSedimento: row.idconcentracaogassedimento,
    dataMedida: row.datamedida,
    horaMedida: row.horamedida,
    batimetria: row.batimetria,
    profundidadeDoSedimento: row.profundidadedosedimento,
    replica: row.replica,
    ch4: row.ch4,
    co2: row.co2,
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        const result = await furnasPool.query(
            `
            SELECT
                a.idConcentracaoGasSedimento,
                a.dataMedida,
                a.horaMedida,
                a.ch4,
                a.co2,
                a.batimetria,
                a.profundidadeDoSedimento,
                a.replica,
                b.idsitio,
        ...
        const countResult = await furnasPool.query("SELECT COUNT(*) FROM tbconcentracaogassedimento",);
        const total = Number(countResult.rows[0].count);

        // Mapeia o resultado para o padrão aninhado, mantendo a listagem concisa
        const data = result.rows.map((row: any) => ({
            ...mapRowToConcentracaoGasSedimento(row),
...
export const getById = async (req: Request, res:Response): Promise<void> => {
    try{
        ...
        if(isNaN(idCGS)){
            res.status(400).json({success: false, error: `ID ${req.params.idConcentracaoGasSedimento} inválido.`,});
            return;
        }
        ...
        if(result.rows.length === 0){
            res.status(404).json({success: false, error:"Registro de concentração de gás no sedimento não encontrado.",});
            return;
        }

        // Mapeia o resultado único para o padrão aninhado
        const data = mapRowToConcentracaoGasSedimento(result.rows[0]);

        res.status(200).json({success: true, data, });

    }   catch(error:any){
        logger.error(`Erro ao buscar registro por ID ${req.params.idConcentracaoGasSedimento} na tabela de concentrações de gás por sedimento.`, 
            {message: error.message, stack: error.stack,});
