...
// Mapeamento
const mapRowToFluxoDifusivoInpe = (row: any) => ({
    idFluxoDifusivoInpe: row.idfluxodifusivoinpe,
    dataMedida: row.datamedida,
    horaMedida: row.horamedida,
    profundidade: row.profundidade,
    co2: row.co2,
    co2_desviopadrao: row.co2_desviopadrao,
    co2_amostras: row.co2_amostras,
    ch4: row.ch4,
    ch4_desviopadrao: row.ch4_desviopadrao,
    ch4_amostras: row.ch4_amostras,
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        const result = await furnasPool.query(
            `
            SELECT
                a.idFluxoDifusivoInpe,
                a.dataMedida,
                a.horaMedida,
                a.profundidade,
                a.co2,
                a.co2_desviopadrao,
                a.ch4, a.ch4_desviopadrao,
                b.idsitio,
        ...
        const countResult = await furnasPool.query("SELECT COUNT(*) FROM tbfluxodifusivoinpe",);
        const total = Number(countResult.rows[0].count);

        // Mapeia o resultado para o padrão aninhado, mantendo a listagem concisa
        const data = result.rows.map((row: any) => ({
            ...mapRowToFluxoDifusivoInpe(row),
            // Remove detalhes extras para o GET ALL
...
export const getById = async (req: Request, res:Response): Promise<void> => {
    try{
        ...
        if(isNaN(idFDInpe)){
            res.status(400).json({success: false, error: `ID ${req.params.idFluxoDifusivoInpe} inválido.`,}); 
            return;
        }
        ...
        if(result.rows.length === 0){
            res.status(404).json({success: false, error:"Registro de dados de fluxo difusivo (INPE) não encontrado.",}); 
            return;
        }

        // Mapeia o resultado único para o padrão aninhado
        const data = mapRowToFluxoDifusivoInpe(result.rows[0]);

        res.status(200).json({success: true, data,});

    }   catch(error:any){
        logger.error(`Erro ao buscar registro por ID ${req.params.idFluxoDifusivoInpe} na tabela de fluxo difusivo (INPE).`, 
            {message: error.message, stack: error.stack,});
