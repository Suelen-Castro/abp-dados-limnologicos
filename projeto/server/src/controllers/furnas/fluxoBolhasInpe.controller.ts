...
// Mapeamento
const mapRowToFluxoBolhasInpe = (row: any) => ({
    idFluxoBolhasInpe: row.idfluxobolhasinpe,
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        const countResult = await furnasPool.query("SELECT COUNT(*) FROM tbfluxobolhasinpe",);
        const total = Number(countResult.rows[0].count);

        // Mapeia o resultado para o padrão aninhado, mantendo a listagem concisa
        const data = result.rows.map((row: any) => ({
            ...mapRowToFluxoBolhasInpe(row),
...
export const getById = async (req: Request, res:Response): Promise<void> => {
    try{
        ...
        if(isNaN(idFBInpe)){
            res.status(400).json({success: false, error: `ID ${req.params.idFluxoBolhasInpe} inválido.`,});
            return;
        }
        ...
        if(result.rows.length === 0){
            res.status(404).json({success: false, error:"Registro de dados de fluxo de bolhas (INPE) não encontrado.",});
            return;
        }

        // Mapeia o resultado único para o padrão aninhado
        const data = mapRowToFluxoBolhasInpe(result.rows[0]);

        res.status(200).json({success: true, data,});

    }   catch(error:any){
        logger.error(`Erro ao buscar registro por ID ${req.params.idFluxoBolhasInpe} na tabela de fluxo de bolhas (INPE).`, 
            {message: error.message, stack: error.stack,});
